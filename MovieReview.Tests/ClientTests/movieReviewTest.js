...
describe("Movie Review Tests -->",function() {
    ...
    describe("dataService-->", function () {
        ...
        it("can load movie reviews", inject(function (movieService) {
            //for the 1st Run
            //expect(dataService.getReviews).toBeDefined();
            expect(movieService.reviews.length).toEqual(0);
        }));
    });
    ...
    describe("Testing Movie Reviews GET Call -->", function () {
        ...
        it("Loaded Movie Reviews", inject(function (movieService) {
            $httpBackend.expectGET(url);
            movieService.getReviews(1);
            $httpBackend.flush();
            expect(movieService.reviews.length).toEqual(5);
        }));
    });
})